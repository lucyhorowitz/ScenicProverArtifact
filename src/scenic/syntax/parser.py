...
# @generated by pegen from ./src/scenic/syntax/scenic.gram
...
class ScenicParser(Parser):
    ...
    def scenic_stmt(self) -> Optional[Any]:
        ...
        self._reset(mark)
        if (
            (scenic_implement_stmt := self.scenic_implement_stmt())
        ):
            return scenic_implement_stmt;
        self._reset(mark)
        return None;

    @memoize
    def scenic_compound_stmt(self) -> Optional[Any]:
        # scenic_compound_stmt: scenic_tracked_assign_new_stmt | scenic_assign_new_stmt | scenic_expr_new_stmt | scenic_behavior_def | scenic_monitor_def | scenic_scenario_def | scenic_try_interrupt_stmt | scenic_override_stmt | scenic_component_def | scenic_contract_def | scenic_assign_contract_test_stmt | scenic_assign_contract_prove_stmt | scenic_assign_contract_assume_stmt | scenic_assign_contract_combination_stmt | scenic_assign_contract_refine_stmt
        mark = self._mark()
        if (
        ...
        self._reset(mark)
        if (
            (scenic_component_def := self.scenic_component_def())
        ):
            return scenic_component_def;
        self._reset(mark)
        if (
            (scenic_contract_def := self.scenic_contract_def())
        ):
            return scenic_contract_def;
        self._reset(mark)
        if (
            (scenic_assign_contract_test_stmt := self.scenic_assign_contract_test_stmt())
        ):
            return scenic_assign_contract_test_stmt;
        self._reset(mark)
        if (
            (scenic_assign_contract_prove_stmt := self.scenic_assign_contract_prove_stmt())
        ):
            return scenic_assign_contract_prove_stmt;
        self._reset(mark)
        if (
            (scenic_assign_contract_assume_stmt := self.scenic_assign_contract_assume_stmt())
        ):
            return scenic_assign_contract_assume_stmt;
        self._reset(mark)
        if (
            (scenic_assign_contract_combination_stmt := self.scenic_assign_contract_combination_stmt())
        ):
            return scenic_assign_contract_combination_stmt;
        self._reset(mark)
        if (
            (scenic_assign_contract_refine_stmt := self.scenic_assign_contract_refine_stmt())
        ):
            return scenic_assign_contract_refine_stmt;
        self._reset(mark)
        return None;
    ...
    @memoize
    def scenic_component_def(self) -> Optional[Any]:
        # scenic_component_def: "component" NAME '(' params? ')' &&':' scenic_component_def_block
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
            ...
            (self.expect_forced(self.expect(':'), "':'"))
            and
            (c := self.scenic_component_def_block())
        ):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return s . ComponentDef ( a . string , args = b or self . make_arguments ( None , [] , None , [] , None ) , docstring = c [0] , sensors = c [1] , inputs = c [2] , outputs = c [3] , actions = c [4] , state = c [5] , body = c [6] , composition = c [7] , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset , );
        self._reset(mark)
        return None;

    @memoize
    def scenic_component_def_block(self) -> Optional[Any]:
        # scenic_component_def_block: NEWLINE INDENT [STRING NEWLINE] scenic_component_sensors_block? scenic_component_inputs_block? scenic_component_outputs_block? scenic_component_actions_block? scenic_component_state_block? scenic_component_body_block? scenic_component_composition_block? DEDENT
        mark = self._mark()
        if (
            (self.expect('NEWLINE'))
            and
            ...
            (a := self._tmp_53(),)
            and
            (b := self.scenic_component_sensors_block(),)
            and
            (c := self.scenic_component_inputs_block(),)
            and
            (d := self.scenic_component_outputs_block(),)
            and
            (e := self.scenic_component_actions_block(),)
            and
            (f := self.scenic_component_state_block(),)
            and
            (g := self.scenic_component_body_block(),)
            and
            (h := self.scenic_component_composition_block(),)
            and
            (self.expect('DEDENT'))
        ):
            return ( a , b or [] , c or [] , d or [] , e or [] , f or [] , g or [] , h or [] );
        self._reset(mark)
        return None;

    @memoize
    def scenic_component_sensors_block(self) -> Optional[Any]:
        # scenic_component_sensors_block: "sensors" &&':' NEWLINE INDENT (scenic_component_sensor)+ DEDENT
        mark = self._mark()
        if (
            (self.expect("sensors"))
            and
    ...
    @memoize
    def scenic_component_state_block(self) -> Optional[Any]:
        # scenic_component_state_block: "state" &&':' NEWLINE INDENT (scenic_component_state)+ DEDENT
        mark = self._mark()
        if (
            (self.expect("state"))
            and
    ...
    @memoize
    def scenic_component_body_block(self) -> Optional[Any]:
        # scenic_component_body_block: "body" &&':' block
        mark = self._mark()
        if (
            (self.expect("body"))
            and
    ...
    @memoize
    def scenic_component_composition_block(self) -> Optional[Any]:
        # scenic_component_composition_block: "compose" &&':' NEWLINE INDENT (scenic_composition_stmt)+ DEDENT
        mark = self._mark()
        if (
            (self.expect("compose"))
            and
    ...
    @memoize
    def scenic_component_sensor(self) -> Optional[Any]:
        # scenic_component_sensor: scenic_component_dot_name ':' primary 'as' NAME NEWLINE
        mark = self._mark()
        if (
            (source := self.scenic_component_dot_name())
            and
            (self.expect(':'))
            and
            (type := self.primary())
            ...
            return ( name . string , source , type );
        self._reset(mark)
        return None;

    @memoize
    def scenic_component_state(self) -> Optional[Any]:
        # scenic_component_state: NAME ':' primary '=' primary NEWLINE
        mark = self._mark()
        if (
            (a := self.name())
            and
            ...
            return ( a . string , val , type );
        self._reset(mark)
        return None;

    @memoize
    def scenic_composition_stmt(self) -> Optional[Any]:
        # scenic_composition_stmt: "connect" scenic_component_dot_name 'to' scenic_component_dot_name NEWLINE | NAME '=' primary NEWLINE
        mark = self._mark()
        if (
            (self.expect("connect"))
            and
            (a := self.scenic_component_dot_name())
            and
            (self.expect('to'))
            and
            (b := self.scenic_component_dot_name())
            and
            (self.expect('NEWLINE'))
        ):
            return ( "connect" , a , b );
        ...
        if (
            ...
            return ( "subcomponent" , a . string , b );
        self._reset(mark)
        return None;

    @memoize
    def scenic_component_dot_name(self) -> Optional[Any]:
        # scenic_component_dot_name: NAME '.' NAME | NAME
        mark = self._mark()
        if (
            (parent := self.name())
            and
    ...
    @memoize
    def scenic_implement_stmt(self) -> Optional[Any]:
        # scenic_implement_stmt: "implement" STRING 'with' primary 'as' NAME
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
            (self.expect("implement"))
            and
            (linked_name := self.string())
            and
            (self.expect('with'))
            and
            ...
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return s . ImplementStmt ( name . string , component , linked_name . string [1 : - 1] , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset );
        self._reset(mark)
        return None;

    @memoize
    def scenic_verify_expr(self) -> Optional[Any]:
        # scenic_verify_expr: "verify" scenic_prefix_operators
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
            ...
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return s . ContractVerify ( e , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset );
        self._reset(mark)
        return None;

    @memoize
    def scenic_contract_def(self) -> Optional[Any]:
        # scenic_contract_def: "contract" NAME '(' params? ')' &&':' scenic_contract_def_block
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
            ...
            (self.expect_forced(self.expect(':'), "':'"))
            and
            (c := self.scenic_contract_def_block())
        ):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return s . ContractDef ( a . string , args = b or self . make_arguments ( None , [] , None , [] , None ) , docstring = c [0] , objects = c [1] , globals = c [2] , inputs = c [3] , outputs = c [4] , actions = c [5] , definitions = c [6] , assumptions = c [7] , guarantees = c [8] , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset , );
        self._reset(mark)
        return None;

    @memoize
    def scenic_contract_def_block(self) -> Optional[Any]:
        # scenic_contract_def_block: NEWLINE INDENT [STRING NEWLINE] scenic_contract_objects_block? scenic_contract_globals_block? scenic_component_inputs_block? scenic_component_outputs_block? scenic_component_actions_block? scenic_contract_definitions_block? scenic_contract_assumptions_block? scenic_contract_guarantees_block? DEDENT
        mark = self._mark()
        if (
            (self.expect('NEWLINE'))
            and
            ...
            (a := self._tmp_57(),)
            and
            (b := self.scenic_contract_objects_block(),)
            and
            (c := self.scenic_contract_globals_block(),)
            and
            (d := self.scenic_component_inputs_block(),)
            and
            (e := self.scenic_component_outputs_block(),)
            and
            (f := self.scenic_component_actions_block(),)
            and
            (g := self.scenic_contract_definitions_block(),)
            and
            (h := self.scenic_contract_assumptions_block(),)
            and
            (i := self.scenic_contract_guarantees_block(),)
            and
            (self.expect('DEDENT'))
        ):
            return ( a , b or ["self"] , c or [] , d or [] , e or [] , f or [] , g or [] , h or [] , i or [] );
        self._reset(mark)
        return None;

    @memoize
    def scenic_contract_objects_block(self) -> Optional[Any]:
        # scenic_contract_objects_block: "objects" &&':' NEWLINE INDENT (scenic_contract_name_line)+ DEDENT
        mark = self._mark()
        if (
            (self.expect("objects"))
            and
    ...
    @memoize
    def scenic_contract_globals_block(self) -> Optional[Any]:
        # scenic_contract_globals_block: "globals" &&':' NEWLINE INDENT (scenic_contract_name_line)+ DEDENT
        mark = self._mark()
        if (
            (self.expect("globals"))
            and
    ...
    @memoize
    def scenic_contract_definitions_block(self) -> Optional[Any]:
        # scenic_contract_definitions_block: "definitions" &&':' NEWLINE INDENT (scenic_contract_definition)+ DEDENT
        mark = self._mark()
        if (
    ...
    @memoize
    def scenic_contract_assumptions_block(self) -> Optional[Any]:
        # scenic_contract_assumptions_block: "assumptions" &&':' NEWLINE INDENT (scenic_contract_temporal_expression_line)+ DEDENT
        mark = self._mark()
        if (
            (self.expect("assumptions"))
            and
            (self.expect_forced(self.expect(':'), "':'"))
            and
            (self.expect('NEWLINE'))
    ...
    @memoize
    def scenic_contract_guarantees_block(self) -> Optional[Any]:
        # scenic_contract_guarantees_block: "guarantees" &&':' NEWLINE INDENT (scenic_contract_temporal_expression_line)+ DEDENT
        mark = self._mark()
        if (
    ...
    @memoize
    def scenic_contract_name_line(self) -> Optional[Any]:
        # scenic_contract_name_line: NAME NEWLINE
        mark = self._mark()
        if (
            (a := self.name())
            and
    ...
    @memoize
    def scenic_contract_definition(self) -> Optional[Any]:
        # scenic_contract_definition: NAME '=' expression NEWLINE
        mark = self._mark()
        if (
            (a := self.name())
            and
    ...
    @memoize
    def scenic_contract_temporal_expression_line(self) -> Optional[Any]:
        # scenic_contract_temporal_expression_line: scenic_temporal_expression NEWLINE
        mark = self._mark()
        if (
    ...
    @memoize
    def scenic_assign_contract_test_stmt(self) -> Optional[Any]:
        # scenic_assign_contract_test_stmt: ((star_targets '='))+ scenic_contract_test_stmt
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
            (a := self._loop1_63())
            and
            (b := self.scenic_contract_test_stmt())
        ):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast . Assign ( targets = a , value = b , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset );
        self._reset(mark)
        return None;

    @memoize
    def scenic_contract_test_stmt(self) -> Optional[Any]:
        # scenic_contract_test_stmt: scenic_contract_test_header ',' NEWLINE scenic_contract_test_block? | scenic_contract_test_header NEWLINE
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
            (a := self.scenic_contract_test_header())
            and
            (self.expect(','))
            and
            (self.expect('NEWLINE'))
            and
            (b := self.scenic_contract_test_block(),)
        ):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return s . ContractTest ( a [0] , a [1] , a [2] + ( b or [] ) , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset );
        self._reset(mark)
        if (
            (a := self.scenic_contract_test_header())
            and
            ...
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return s . ContractTest ( a [0] , a [1] , a [2] , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset );
        self._reset(mark)
        return None;

    @memoize
    def scenic_contract_test_header(self) -> Optional[Any]:
        # scenic_contract_test_header: "test" scenic_subcomponent_name "satisfies" primary [',' scenic_contract_test_specs]
        mark = self._mark()
        if (
            (self.expect("test"))
            and
            (component := self.scenic_subcomponent_name())
            and
            (self.expect("satisfies"))
            and
            (contract := self.primary())
            and
            (specs := self._tmp_64(),)
        ):
            return ( contract , component , ( specs or [] ) );
        self._reset(mark)
        return None;

    @memoize
    def scenic_contract_test_block(self) -> Optional[Any]:
        # scenic_contract_test_block: INDENT scenic_contract_test_block_inner DEDENT
        mark = self._mark()
        if (
            (self.expect('INDENT'))
            and
            (b := self.scenic_contract_test_block_inner())
            and
            (self.expect('DEDENT'))
        ):
            return b;
    ...
    @memoize
    def scenic_contract_test_block_inner(self) -> Optional[Any]:
        # scenic_contract_test_block_inner: ((scenic_contract_test_specs ',' NEWLINE))* scenic_contract_test_specs NEWLINE | ((scenic_contract_test_specs ',' NEWLINE))+
        mark = self._mark()
        if (
            (xss := self._loop0_65(),)
            and
            (c := self.scenic_contract_test_specs())
            and
            (self.expect('NEWLINE'))
        ):
            return ( [x for xs in xss for x in xs] or [] ) + c;
        self._reset(mark)
        if (
            (xss := self._loop1_66())
        ):
    ...
    @memoize
    def scenic_contract_test_specs(self) -> Optional[Any]:
        # scenic_contract_test_specs: ','.scenic_contract_test_spec+
        mark = self._mark()
        if (
            (a := self._gather_67())
        ):
    ...
    @memoize
    def scenic_contract_test_spec(self) -> Optional[Any]:
        # scenic_contract_test_spec: "using" primary | scenic_contract_test_term_cond | scenic_contract_test_req_cond
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
            (self.expect("using"))
            and
            (technique := self.primary())
        ):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return s . ContractTestTechnique ( technique , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset );
        self._reset(mark)
        if (
            (tc := self.scenic_contract_test_term_cond())
        ):
            return tc;
        self._reset(mark)
        if (
            (rc := self.scenic_contract_test_req_cond())
        ):
            return rc;
        self._reset(mark)
        return None;
    ...
    @memoize
    def scenic_contract_test_term_cond(self) -> Optional[Any]:
        # scenic_contract_test_term_cond: "terminating" "after" expression "seconds" | "terminating" "after" expression "minutes" | "terminating" "after" expression "hours" | "terminating" "after" expression "samples" | "terminating" "with" expression "gap"
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
            (self.expect("terminating"))
            and
            (self.expect("after"))
            and
            (x := self.expression())
            ...
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return s . ContractTestTermCond ( "time" , [x] , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset );
        self._reset(mark)
        if (
            (self.expect("terminating"))
            and
            ...
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return s . ContractTestTermCond ( "time" , [ast . Mult ( ast . Constant ( 60 ) , x )] , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset );
        self._reset(mark)
        if (
            (self.expect("terminating"))
            and
            ...
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return s . ContractTestTermCond ( "time" , [ast . Mult ( ast . Constant ( 60 * 60 ) , x )] , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset );
        self._reset(mark)
        if (
            (self.expect("terminating"))
            and
            ...
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return s . ContractTestTermCond ( "samples" , [x] , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset );
        self._reset(mark)
        if (
            (self.expect("terminating"))
            and
            ...
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return s . ContractTestTermCond ( "gap" , [x] , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset );
        self._reset(mark)
        return None;

    @memoize
    def scenic_contract_test_req_cond(self) -> Optional[Any]:
        # scenic_contract_test_req_cond: "requiring" expression "correctness"
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
            (self.expect("requiring"))
            and
            (x := self.expression())
            and
            (self.expect("correctness"))
        ):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return s . ContractTestReqCond ( "correctness" , [x] , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset );
        self._reset(mark)
        return None;

    @memoize
    def scenic_assign_contract_prove_stmt(self) -> Optional[Any]:
        # scenic_assign_contract_prove_stmt: ((star_targets '='))+ scenic_contract_prove_stmt
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
            (a := self._loop1_69())
            and
            (b := self.scenic_contract_prove_stmt())
        ):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast . Assign ( targets = a , value = b , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset );
    ...
    @memoize
    def scenic_contract_prove_stmt(self) -> Optional[Any]:
        # scenic_contract_prove_stmt: scenic_contract_prove_header NEWLINE
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
            (a := self.scenic_contract_prove_header())
            and
            (self.expect('NEWLINE'))
        ):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return s . ContractProof ( a [0] , a [1] , a [2] , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset );
        self._reset(mark)
        return None;

    @memoize
    def scenic_contract_prove_header(self) -> Optional[Any]:
        # scenic_contract_prove_header: "prove" scenic_subcomponent_name "satisfies" primary "using" primary
        mark = self._mark()
        if (
            (self.expect("prove"))
            and
            ...
            return ( contract , component , method );
        self._reset(mark)
        return None;

    @memoize
    def scenic_assign_contract_assume_stmt(self) -> Optional[Any]:
        # scenic_assign_contract_assume_stmt: ((star_targets '='))+ scenic_contract_assume_stmt
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
            (a := self._loop1_70())
            and
            (b := self.scenic_contract_assume_stmt())
        ):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast . Assign ( targets = a , value = b , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset );
    ...
    @memoize
    def scenic_contract_assume_stmt(self) -> Optional[Any]:
        # scenic_contract_assume_stmt: scenic_contract_assume_header ',' NEWLINE scenic_contract_assume_block? | scenic_contract_assume_header NEWLINE
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
            (a := self.scenic_contract_assume_header())
            and
            (self.expect(','))
            and
            (self.expect('NEWLINE'))
            and
            (b := self.scenic_contract_assume_block(),)
        ):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return s . ContractAssume ( a [0] , a [1] , a [2] + ( b or [] ) , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset );
        self._reset(mark)
        if (
            (a := self.scenic_contract_assume_header())
            and
            ...
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return s . ContractAssume ( a [0] , a [1] , a [2] , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset );
        self._reset(mark)
        return None;

    @memoize
    def scenic_contract_assume_header(self) -> Optional[Any]:
        # scenic_contract_assume_header: "assume" scenic_subcomponent_name "satisfies" primary [',' scenic_contract_assume_specs]
        mark = self._mark()
        if (
            (self.expect("assume"))
            and
    ...
    @memoize
    def scenic_contract_assume_block(self) -> Optional[Any]:
        # scenic_contract_assume_block: INDENT scenic_contract_assume_block_inner DEDENT
        mark = self._mark()
        if (
            (self.expect('INDENT'))
            and
            (b := self.scenic_contract_assume_block_inner())
            and
            (self.expect('DEDENT'))
        ):
            return b;
    ...
    @memoize
    def scenic_contract_assume_block_inner(self) -> Optional[Any]:
        # scenic_contract_assume_block_inner: ((scenic_contract_assume_specs ',' NEWLINE))* scenic_contract_assume_specs NEWLINE | ((scenic_contract_assume_specs ',' NEWLINE))+
        mark = self._mark()
        if (
            (xss := self._loop0_72(),)
            and
            (c := self.scenic_contract_assume_specs())
            and
            (self.expect('NEWLINE'))
        ):
            return ( [x for xs in xss for x in xs] or [] ) + c;
    ...
    @memoize
    def scenic_contract_assume_specs(self) -> Optional[Any]:
        # scenic_contract_assume_specs: ','.scenic_contract_assume_spec+
        mark = self._mark()
        if (
            (a := self._gather_74())
        ):
    ...
    @memoize
    def scenic_contract_assume_spec(self) -> Optional[Any]:
        # scenic_contract_assume_spec: "with" "correctness" expression | "with" "confidence" expression
        mark = self._mark()
        if (
            (self.expect("with"))
            and
    ...
    @memoize
    def scenic_assign_contract_combination_stmt(self) -> Optional[Any]:
        # scenic_assign_contract_combination_stmt: ((star_targets '='))+ (scenic_contract_compose_stmt | scenic_contract_wmerge_stmt)
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
    ...
    @memoize
    def scenic_contract_compose_stmt(self) -> Optional[Any]:
        # scenic_contract_compose_stmt: "compose" "over" scenic_subcomponent_name ':' NEWLINE INDENT (scenic_contract_combination_inner)+ DEDENT
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
            ...
            (self.expect("over"))
            and
            (name := self.scenic_subcomponent_name())
            and
            (self.expect(':'))
            and
            (self.expect('NEWLINE'))
            ...
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return s . ContractCompose ( name , a , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset );
        self._reset(mark)
        return None;

    @memoize
    def scenic_contract_wmerge_stmt(self) -> Optional[Any]:
        # scenic_contract_wmerge_stmt: "weak" "merge" "over" scenic_subcomponent_name ':' NEWLINE INDENT (scenic_contract_combination_inner)+ DEDENT
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
            ...
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return s . ContractWeakMerge ( name , a , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset );
        self._reset(mark)
        return None;

    @memoize
    def scenic_contract_combination_inner(self) -> Optional[Any]:
        # scenic_contract_combination_inner: scenic_contract_test_stmt | scenic_contract_prove_stmt | scenic_contract_assume_stmt | scenic_contract_use_stmt
        mark = self._mark()
        if (
            (scenic_contract_test_stmt := self.scenic_contract_test_stmt())
        ):
            return scenic_contract_test_stmt;
        self._reset(mark)
        if (
            (scenic_contract_prove_stmt := self.scenic_contract_prove_stmt())
        ):
            return scenic_contract_prove_stmt;
        self._reset(mark)
        if (
            (scenic_contract_assume_stmt := self.scenic_contract_assume_stmt())
        ):
            return scenic_contract_assume_stmt;
        self._reset(mark)
        if (
            (scenic_contract_use_stmt := self.scenic_contract_use_stmt())
        ):
            return scenic_contract_use_stmt;
        self._reset(mark)
        return None;

    @memoize
    def scenic_contract_use_stmt(self) -> Optional[Any]:
        # scenic_contract_use_stmt: "use" NAME NEWLINE
        mark = self._mark()
        if (
            (self.expect("use"))
            and
            ...
            return s . ContractUse ( component . string );
        self._reset(mark)
        return None;

    @memoize
    def scenic_assign_contract_refine_stmt(self) -> Optional[Any]:
        # scenic_assign_contract_refine_stmt: ((star_targets '='))+ scenic_contract_refine_stmt
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
            (a := self._loop1_80())
            and
            (b := self.scenic_contract_refine_stmt())
        ):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast . Assign ( targets = a , value = b , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset );
    ...
    @memoize
    def scenic_contract_refine_stmt(self) -> Optional[Any]:
        # scenic_contract_refine_stmt: "refine" NAME "as" primary "using" primary NEWLINE
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
            ...
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return s . ContractRefine ( name . string , contract , method , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset );
        self._reset(mark)
        return None;

    @memoize
    def scenic_component_inputs_block(self) -> Optional[Any]:
        # scenic_component_inputs_block: "inputs" &&':' NEWLINE INDENT (scenic_component_port)+ DEDENT
        mark = self._mark()
        if (
            (self.expect("inputs"))
            and
    ...
    @memoize
    def scenic_component_outputs_block(self) -> Optional[Any]:
        # scenic_component_outputs_block: "outputs" &&':' NEWLINE INDENT (scenic_component_port)+ DEDENT
        mark = self._mark()
        if (
            (self.expect("outputs"))
            and
    ...
    @memoize
    def scenic_component_actions_block(self) -> Optional[Any]:
        # scenic_component_actions_block: "actions" &&':' NEWLINE INDENT (scenic_component_port)+ DEDENT
        mark = self._mark()
        if (
            (self.expect("actions"))
            and
    ...
    @memoize
    def scenic_component_port(self) -> Optional[Any]:
        # scenic_component_port: NAME ':' primary NEWLINE
        mark = self._mark()
        if (
            (a := self.name())
            and
    ...
    @memoize
    def scenic_subcomponent_name(self) -> Optional[Any]:
        # scenic_subcomponent_name: NAME '.' scenic_subcomponent_name | NAME
        mark = self._mark()
        if (
            (parent := self.name())
            and
            (self.expect('.'))
            and
            (child := self.scenic_subcomponent_name())
        ):
            return ( parent . string , ) + child;
        self._reset(mark)
        if (
            (a := self.name())
        ):
    ...
    def literal_pattern(self) -> Optional[Any]:
        ...
        if (
            ...
            and
            (self.negative_lookahead(self._tmp_119, ))
        ):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
    ...
    def literal_expr(self) -> Optional[Any]:
        ...
        if (
            ...
            and
            (self.negative_lookahead(self._tmp_120, ))
        ):
    ...
    def pattern_capture_target(self) -> Optional[str]:
        ...
        if (
            ...
            (name := self.name())
            and
            (self.negative_lookahead(self._tmp_121, ))
        ):
            return name . string;
        self._reset(mark)
        return None;
    ...
    def value_pattern(self) -> Optional["ast.MatchValue"]:
        ...
        if (
            (attr := self.attr())
            and
            (self.negative_lookahead(self._tmp_122, ))
        ):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
    ...
    def scenic_temporal_group(self) -> Optional[Any]:
        ...
        if (
            ...
            (self.expect(')'))
            and
            (self.positive_lookahead(self._tmp_146, ))
        ):
            return a;
        self._reset(mark)
        return None;
    ...
    def scenic_prefix_operators(self) -> Optional[Any]:
        ...
        if (
            ...
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return s . ContractNext ( e , 1 , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset );
        self._reset(mark)
        if (
            (scenic_verify_expr := self.scenic_verify_expr())
        ):
            return scenic_verify_expr;
        self._reset(mark)
        if (
            (sum := self.sum())
        ):
    ...
    def kwargs(self) -> Optional[list]:
        ...
        self._reset(mark)
        if (
            (_gather_223 := self._gather_223())
        ):
            return _gather_223;
        self._reset(mark)
        if (
            (_gather_225 := self._gather_225())
        ):
            return _gather_225;
        self._reset(mark)
        return None;
    ...
    def invalid_expression(self) -> Optional[NoReturn]:
        ...
        mark = self._mark()
        if (
            (self.negative_lookahead(self._tmp_248, ))
            and
            (a := self.disjunction())
            and
        ...
        if (
            ...
            (b := self.disjunction())
            and
            (self.negative_lookahead(self._tmp_249, ))
        ):
        ...
        if (
            ...
            (b := self.expect(':'))
            and
            (self.positive_lookahead(self._tmp_250, ))
        ):
    ...
    def invalid_named_expression(self) -> Optional[NoReturn]:
        ...
        self._reset(mark)
        if (
            (self.negative_lookahead(self._tmp_252, ))
            and
            (a := self.bitwise_or())
            and
            (self.expect('='))
            ...
            (self.bitwise_or())
            and
            (self.negative_lookahead(self._tmp_253, ))
        ):
    ...
    def invalid_scenic_require_prob(self) -> Optional[NoReturn]:
        ...
        if (
            ...
            (self.expect('['))
            and
            (self.negative_lookahead(self._tmp_254, ))
            and
            (p := self.expression())
            and
            (self.expect(']'))
    ...
    def invalid_default(self) -> Optional[Any]:
        ...
        if (
            (a := self.expect('='))
            and
            (self.positive_lookahead(self._tmp_273, ))
        ):
    ...
    def invalid_with_item(self) -> Optional[NoReturn]:
        ...
        if (
            ...
            (a := self.expression())
            and
            (self.positive_lookahead(self._tmp_297, ))
        ):
            return self . raise_syntax_error_invalid_target ( Target . STAR_TARGETS , a );
        self._reset(mark)
        return None;
    ...
    def invalid_double_starred_kvpairs(self) -> Optional[None]:
        ...
        if (
            ...
            (a := self.expect(':'))
            and
            (self.positive_lookahead(self._tmp_326, ))
        ):
    ...
    def invalid_kvpair(self) -> Optional[None]:
        ...
        if (
            ...
            (a := self.expect(':'))
            and
            (self.positive_lookahead(self._tmp_327, ))
        ):
            return self . raise_syntax_error_known_location ( "expression expected after dictionary key and ':'" , a );
        self._reset(mark)
        if (
    ...
    def invalid_replacement_field(self) -> Optional[Any]:
        ...
        if (
            (self.expect('{'))
            and
            (self.negative_lookahead(self._tmp_328, ))
        ):
        ...
        if (
            ...
            (self._tmp_329())
            and
            (self.negative_lookahead(self._tmp_330, ))
        ):
        ...
        if (
            ...
            (self.expect('='))
            and
            (self.negative_lookahead(self._tmp_332, ))
        ):
        ...
        if (
            ...
            (self._tmp_335(),)
            and
            (self.negative_lookahead(self._tmp_336, ))
        ):
    ...
    def invalid_conversion_character(self) -> Optional[Any]:
        ...
        if (
            (self.expect('!'))
            and
            (self.positive_lookahead(self._tmp_342, ))
        ):
    ...
    def _loop1_17(self) -> Optional[Any]:
        ...
        children = []
        while (
            (_tmp_343 := self._tmp_343())
        ):
            children.append(_tmp_343)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _loop0_27(self) -> Optional[Any]:
        ...
        children = []
        while (
            (_tmp_344 := self._tmp_344())
        ):
            children.append(_tmp_344)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _loop1_28(self) -> Optional[Any]:
        ...
        children = []
        while (
            (_tmp_345 := self._tmp_345())
        ):
            children.append(_tmp_345)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _loop1_42(self) -> Optional[Any]:
        ...
        children = []
        while (
            (_tmp_348 := self._tmp_348())
        ):
            children.append(_tmp_348)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _loop1_44(self) -> Optional[Any]:
        ...
        children = []
        while (
            (_tmp_350 := self._tmp_350())
        ):
            children.append(_tmp_350)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _loop1_47(self) -> Optional[Any]:
        ...
        children = []
        while (
            (_tmp_351 := self._tmp_351())
        ):
            children.append(_tmp_351)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _loop1_54(self) -> Optional[Any]:
        # _loop1_54: (scenic_component_sensor)
        mark = self._mark()
        children = []
        while (
            (scenic_component_sensor := self.scenic_component_sensor())
        ):
            children.append(scenic_component_sensor)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _loop1_55(self) -> Optional[Any]:
        # _loop1_55: (scenic_component_state)
        mark = self._mark()
        children = []
        while (
            (scenic_component_state := self.scenic_component_state())
        ):
            children.append(scenic_component_state)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _loop1_56(self) -> Optional[Any]:
        # _loop1_56: (scenic_composition_stmt)
        mark = self._mark()
        children = []
        while (
            (scenic_composition_stmt := self.scenic_composition_stmt())
        ):
            children.append(scenic_composition_stmt)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _loop1_58(self) -> Optional[Any]:
        # _loop1_58: (scenic_contract_name_line)
        mark = self._mark()
        children = []
        while (
            (scenic_contract_name_line := self.scenic_contract_name_line())
        ):
            children.append(scenic_contract_name_line)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _loop1_59(self) -> Optional[Any]:
        # _loop1_59: (scenic_contract_name_line)
        mark = self._mark()
        children = []
        while (
            (scenic_contract_name_line := self.scenic_contract_name_line())
    ...
    def _loop1_60(self) -> Optional[Any]:
        # _loop1_60: (scenic_contract_definition)
        mark = self._mark()
        children = []
        while (
            (scenic_contract_definition := self.scenic_contract_definition())
        ):
            children.append(scenic_contract_definition)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _loop1_61(self) -> Optional[Any]:
        # _loop1_61: (scenic_contract_temporal_expression_line)
        mark = self._mark()
        children = []
        while (
            (scenic_contract_temporal_expression_line := self.scenic_contract_temporal_expression_line())
        ):
            children.append(scenic_contract_temporal_expression_line)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _loop1_62(self) -> Optional[Any]:
        # _loop1_62: (scenic_contract_temporal_expression_line)
        mark = self._mark()
        children = []
        while (
            (scenic_contract_temporal_expression_line := self.scenic_contract_temporal_expression_line())
    ...
    def _loop1_63(self) -> Optional[Any]:
        # _loop1_63: (star_targets '=')
        mark = self._mark()
        children = []
        while (
            (_tmp_352 := self._tmp_352())
        ):
            children.append(_tmp_352)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _tmp_64(self) -> Optional[Any]:
        # _tmp_64: ',' scenic_contract_test_specs
        mark = self._mark()
        if (
            (self.expect(','))
            and
            (x := self.scenic_contract_test_specs())
        ):
            return x;
        self._reset(mark)
        return None;
    ...
    def _loop0_65(self) -> Optional[Any]:
        # _loop0_65: (scenic_contract_test_specs ',' NEWLINE)
        mark = self._mark()
        children = []
        while (
            (_tmp_353 := self._tmp_353())
        ):
            children.append(_tmp_353)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _loop1_66(self) -> Optional[Any]:
        # _loop1_66: (scenic_contract_test_specs ',' NEWLINE)
        mark = self._mark()
        children = []
        while (
            (_tmp_354 := self._tmp_354())
        ):
            children.append(_tmp_354)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _loop0_68(self) -> Optional[Any]:
        # _loop0_68: ',' scenic_contract_test_spec
        mark = self._mark()
        children = []
        while (
            (self.expect(','))
            and
            (elem := self.scenic_contract_test_spec())
        ):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
    ...
    @memoize
    def _gather_67(self) -> Optional[Any]:
        # _gather_67: scenic_contract_test_spec _loop0_68
        mark = self._mark()
        if (
            (elem := self.scenic_contract_test_spec())
            is not None
    ...
    def _loop1_69(self) -> Optional[Any]:
        # _loop1_69: (star_targets '=')
        mark = self._mark()
        children = []
        while (
            (_tmp_355 := self._tmp_355())
        ):
            children.append(_tmp_355)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _loop1_70(self) -> Optional[Any]:
        # _loop1_70: (star_targets '=')
        mark = self._mark()
        children = []
        while (
            (_tmp_356 := self._tmp_356())
        ):
            children.append(_tmp_356)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _tmp_71(self) -> Optional[Any]:
        # _tmp_71: ',' scenic_contract_assume_specs
        mark = self._mark()
        if (
            (self.expect(','))
            and
            (x := self.scenic_contract_assume_specs())
        ):
            return x;
        self._reset(mark)
        return None;
    ...
    def _loop0_72(self) -> Optional[Any]:
        # _loop0_72: (scenic_contract_assume_specs ',' NEWLINE)
        mark = self._mark()
        children = []
        while (
            (_tmp_357 := self._tmp_357())
        ):
            children.append(_tmp_357)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _loop1_73(self) -> Optional[Any]:
        # _loop1_73: (scenic_contract_assume_specs ',' NEWLINE)
        mark = self._mark()
        children = []
        while (
            (_tmp_358 := self._tmp_358())
        ):
            children.append(_tmp_358)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _loop0_75(self) -> Optional[Any]:
        # _loop0_75: ',' scenic_contract_assume_spec
        mark = self._mark()
        children = []
        while (
            (self.expect(','))
            and
            (elem := self.scenic_contract_assume_spec())
        ):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
    ...
    def _gather_74(self) -> Optional[Any]:
        # _gather_74: scenic_contract_assume_spec _loop0_75
        mark = self._mark()
        if (
            (elem := self.scenic_contract_assume_spec())
            is not None
    ...
    def _loop1_76(self) -> Optional[Any]:
        # _loop1_76: (star_targets '=')
        mark = self._mark()
        children = []
        while (
            (_tmp_359 := self._tmp_359())
        ):
            children.append(_tmp_359)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _tmp_77(self) -> Optional[Any]:
        # _tmp_77: scenic_contract_compose_stmt | scenic_contract_wmerge_stmt
        mark = self._mark()
        if (
            (scenic_contract_compose_stmt := self.scenic_contract_compose_stmt())
        ):
            return scenic_contract_compose_stmt;
        self._reset(mark)
        if (
            (scenic_contract_wmerge_stmt := self.scenic_contract_wmerge_stmt())
        ):
            return scenic_contract_wmerge_stmt;
        self._reset(mark)
        return None;

    @memoize
    def _loop1_78(self) -> Optional[Any]:
        # _loop1_78: (scenic_contract_combination_inner)
        mark = self._mark()
        children = []
        while (
            (scenic_contract_combination_inner := self.scenic_contract_combination_inner())
        ):
            children.append(scenic_contract_combination_inner)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _loop1_79(self) -> Optional[Any]:
        # _loop1_79: (scenic_contract_combination_inner)
        mark = self._mark()
        children = []
        while (
            (scenic_contract_combination_inner := self.scenic_contract_combination_inner())
    ...
    def _loop1_80(self) -> Optional[Any]:
        # _loop1_80: (star_targets '=')
        mark = self._mark()
        children = []
        while (
            (_tmp_360 := self._tmp_360())
        ):
            children.append(_tmp_360)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _loop1_81(self) -> Optional[Any]:
        # _loop1_81: (scenic_component_port)
        mark = self._mark()
        children = []
        while (
            (scenic_component_port := self.scenic_component_port())
        ):
            children.append(scenic_component_port)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _loop1_82(self) -> Optional[Any]:
        # _loop1_82: (scenic_component_port)
        mark = self._mark()
        children = []
        while (
            (scenic_component_port := self.scenic_component_port())
    ...
    def _loop1_83(self) -> Optional[Any]:
        # _loop1_83: (scenic_component_port)
        mark = self._mark()
        children = []
        while (
            (scenic_component_port := self.scenic_component_port())
    ...
    def _loop1_100(self) -> Optional[Any]:
        # _loop1_100: param_maybe_default
        mark = self._mark()
        children = []
        while (
            (param_maybe_default := self.param_maybe_default())
    ...
    def _gather_101(self) -> Optional[Any]:
        # _gather_101: with_item _loop0_102
        mark = self._mark()
        if (
            (elem := self.with_item())
            is not None
    ...
    def _gather_103(self) -> Optional[Any]:
        # _gather_103: with_item _loop0_104
        mark = self._mark()
        if (
            (elem := self.with_item())
            is not None
    ...
    def _gather_105(self) -> Optional[Any]:
        # _gather_105: with_item _loop0_106
        mark = self._mark()
        if (
            (elem := self.with_item())
            is not None
    ...
    def _gather_107(self) -> Optional[Any]:
        # _gather_107: with_item _loop0_108
        mark = self._mark()
        if (
            (elem := self.with_item())
            is not None
    ...
    def _loop1_112(self) -> Optional[Any]:
        # _loop1_112: interrupt_when_block
        mark = self._mark()
        children = []
        while (
    ...
    def _gather_117(self) -> Optional[Any]:
        # _gather_117: closed_pattern _loop0_118
        mark = self._mark()
        if (
            (elem := self.closed_pattern())
            is not None
    ...
    def _gather_123(self) -> Optional[Any]:
        # _gather_123: maybe_star_pattern _loop0_124
        mark = self._mark()
        if (
            (elem := self.maybe_star_pattern())
            is not None
    ...
    def _gather_125(self) -> Optional[Any]:
        # _gather_125: key_value_pattern _loop0_126
        mark = self._mark()
        if (
            (elem := self.key_value_pattern())
            is not None
    ...
    def _gather_128(self) -> Optional[Any]:
        # _gather_128: pattern _loop0_129
        mark = self._mark()
        if (
            (elem := self.pattern())
            is not None
    ...
    def _gather_130(self) -> Optional[Any]:
        # _gather_130: keyword_pattern _loop0_131
        mark = self._mark()
        if (
            (elem := self.keyword_pattern())
            is not None
    ...
    def _gather_132(self) -> Optional[Any]:
        # _gather_132: type_param _loop0_133
        mark = self._mark()
        if (
            (elem := self.type_param())
            is not None
    ...
    def _loop1_134(self) -> Optional[Any]:
        # _loop1_134: (',' expression)
        mark = self._mark()
        children = []
        while (
            (_tmp_361 := self._tmp_361())
        ):
            children.append(_tmp_361)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _loop1_135(self) -> Optional[Any]:
        # _loop1_135: (',' star_expression)
        mark = self._mark()
        children = []
        while (
            (_tmp_362 := self._tmp_362())
        ):
            children.append(_tmp_362)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _loop0_137(self) -> Optional[Any]:
        # _loop0_137: ',' star_named_expression
        mark = self._mark()
        children = []
        while (
            (self.expect(','))
    ...
    def _gather_136(self) -> Optional[Any]:
        # _gather_136: star_named_expression _loop0_137
        mark = self._mark()
        if (
            (elem := self.star_named_expression())
            is not None
    ...
    def _tmp_138(self) -> Optional[Any]:
        # _tmp_138: scenic_temporal_prefix | scenic_temporal_disjunction
        mark = self._mark()
        if (
            (scenic_temporal_prefix := self.scenic_temporal_prefix())
        ):
    ...
    def _loop1_139(self) -> Optional[Any]:
        # _loop1_139: ('or' conjunction)
        mark = self._mark()
        children = []
        while (
            (_tmp_363 := self._tmp_363())
        ):
            children.append(_tmp_363)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _loop1_140(self) -> Optional[Any]:
        # _loop1_140: ('or' (scenic_temporal_prefix | scenic_temporal_conjunction))
        mark = self._mark()
        children = []
        while (
            (_tmp_364 := self._tmp_364())
        ):
            children.append(_tmp_364)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _loop1_141(self) -> Optional[Any]:
        # _loop1_141: ('and' inversion)
        mark = self._mark()
        children = []
        while (
            (_tmp_365 := self._tmp_365())
        ):
            children.append(_tmp_365)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _loop1_142(self) -> Optional[Any]:
        # _loop1_142: ('and' (scenic_temporal_prefix | scenic_temporal_inversion))
        mark = self._mark()
        children = []
        while (
            (_tmp_366 := self._tmp_366())
        ):
            children.append(_tmp_366)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _tmp_144(self) -> Optional[Any]:
        # _tmp_144: "visible" scenic_temporal_inversion
        mark = self._mark()
        if (
            (literal := self.expect("visible"))
            and
    ...
    def _tmp_145(self) -> Optional[Any]:
        # _tmp_145: scenic_temporal_prefix | scenic_temporal_inversion
        mark = self._mark()
        if (
            (scenic_temporal_prefix := self.scenic_temporal_prefix())
        ):
    ...
    def _gather_147(self) -> Optional[Any]:
        # _gather_147: scenic_specifier _loop0_148
        mark = self._mark()
        if (
            (elem := self.scenic_specifier())
            is not None
    ...
    def _loop1_155(self) -> Optional[Any]:
        # _loop1_155: compare_op_bitwise_or_pair
        mark = self._mark()
        children = []
        while (
    ...
    def _tmp_156(self) -> Optional[Any]:
        # _tmp_156: "relative" 'to' | "offset" 'by'
        mark = self._mark()
        if (
    ...
    def _loop0_159(self) -> Optional[Any]:
        # _loop0_159: ',' (slice | starred_expression)
        mark = self._mark()
        children = []
        while (
            (self.expect(','))
    ...
    def _gather_158(self) -> Optional[Any]:
        # _gather_158: (slice | starred_expression) _loop0_159
        mark = self._mark()
        if (
            (elem := self._tmp_367())
            is not None
    ...
    def _tmp_164(self) -> Optional[Any]:
        # _tmp_164: dict | set | dictcomp | setcomp
        mark = self._mark()
        if (
            (dict := self.dict())
        ):
    ...
    def _tmp_165(self) -> Optional[Any]:
        # _tmp_165: yield_expr | named_expression
        mark = self._mark()
        if (
            (yield_expr := self.yield_expr())
        ):
    ...
    def _loop0_167(self) -> Optional[Any]:
        # _loop0_167: lambda_param_with_default
        mark = self._mark()
        children = []
        while (
    ...
    def _loop0_168(self) -> Optional[Any]:
        # _loop0_168: lambda_param_with_default
        mark = self._mark()
        children = []
        while (
            (lambda_param_with_default := self.lambda_param_with_default())
    ...
    def _loop1_169(self) -> Optional[Any]:
        # _loop1_169: lambda_param_no_default
        mark = self._mark()
        children = []
        while (
            (lambda_param_no_default := self.lambda_param_no_default())
    ...
    def _loop0_170(self) -> Optional[Any]:
        # _loop0_170: lambda_param_with_default
        mark = self._mark()
        children = []
        while (
            (lambda_param_with_default := self.lambda_param_with_default())
    ...
    @memoize
    def _loop1_171(self) -> Optional[Any]:
        # _loop1_171: lambda_param_with_default
        mark = self._mark()
        children = []
        while (
            (lambda_param_with_default := self.lambda_param_with_default())
    ...
    def _loop1_172(self) -> Optional[Any]:
        # _loop1_172: lambda_param_no_default
        mark = self._mark()
        children = []
        while (
            (lambda_param_no_default := self.lambda_param_no_default())
    ...
    def _loop1_173(self) -> Optional[Any]:
        # _loop1_173: lambda_param_no_default
        mark = self._mark()
        children = []
        while (
            (lambda_param_no_default := self.lambda_param_no_default())
    ...
    def _loop0_178(self) -> Optional[Any]:
        # _loop0_178: lambda_param_maybe_default
        mark = self._mark()
        children = []
        while (
    ...
    @memoize
    def _loop1_179(self) -> Optional[Any]:
        # _loop1_179: lambda_param_maybe_default
        mark = self._mark()
        children = []
        while (
            (lambda_param_maybe_default := self.lambda_param_maybe_default())
    ...
    def _loop0_181(self) -> Optional[Any]:
        # _loop0_181: ',' scenic_param_stmt_param
        mark = self._mark()
        children = []
        while (
            (self.expect(','))
    ...
    def _gather_180(self) -> Optional[Any]:
        # _gather_180: scenic_param_stmt_param _loop0_181
        mark = self._mark()
        if (
            (elem := self.scenic_param_stmt_param())
            is not None
    ...
    def _tmp_182(self) -> Optional[Any]:
        # _tmp_182: 'as' scenic_require_stmt_name
        mark = self._mark()
        if (
    ...
    def _tmp_184(self) -> Optional[Any]:
        # _tmp_184: 'as' scenic_require_stmt_name
        mark = self._mark()
        if (
            (self.expect('as'))
            and
    ...
    def _tmp_186(self) -> Optional[Any]:
        # _tmp_186: 'as' scenic_require_stmt_name
        mark = self._mark()
        if (
            (self.expect('as'))
            and
    ...
    def _tmp_187(self) -> Optional[Any]:
        # _tmp_187: 'as' scenic_require_stmt_name
        mark = self._mark()
        if (
            (self.expect('as'))
            and
    ...
    def _tmp_188(self) -> Optional[Any]:
        # _tmp_188: 'as' scenic_require_stmt_name
        mark = self._mark()
        if (
            (self.expect('as'))
            and
    ...
    def _loop0_190(self) -> Optional[Any]:
        # _loop0_190: ',' scenic_mutate_stmt_id
        mark = self._mark()
        children = []
        while (
            (self.expect(','))
    ...
    @memoize
    def _gather_189(self) -> Optional[Any]:
        # _gather_189: scenic_mutate_stmt_id _loop0_190
        mark = self._mark()
        if (
            (elem := self.scenic_mutate_stmt_id())
            is not None
    ...
    def _gather_192(self) -> Optional[Any]:
        # _gather_192: expression _loop0_193
        mark = self._mark()
        if (
            (elem := self.expression())
            is not None
    ...
    def _tmp_194(self) -> Optional[Any]:
        # _tmp_194: 'as' scenic_require_stmt_name
        mark = self._mark()
        if (
            (self.expect('as'))
            and
    ...
    def _tmp_195(self) -> Optional[Any]:
        # _tmp_195: 'as' scenic_require_stmt_name
        mark = self._mark()
        if (
            (self.expect('as'))
            and
    ...
    @memoize
    def _gather_202(self) -> Optional[Any]:
        # _gather_202: disjunction _loop0_203
        mark = self._mark()
        if (
            (elem := self.disjunction())
            is not None
    ...
    def _gather_204(self) -> Optional[Any]:
        # _gather_204: expression _loop0_205
        mark = self._mark()
        if (
            (elem := self.expression())
            is not None
    ...
    def _tmp_206(self) -> Optional[Any]:
        # _tmp_206: yield_expr | star_expressions
        mark = self._mark()
        if (
            (yield_expr := self.yield_expr())
        ):
    ...
    def _loop0_207(self) -> Optional[Any]:
        # _loop0_207: fstring_format_spec
        mark = self._mark()
        children = []
        while (
    ...
    @memoize
    def _loop1_208(self) -> Optional[Any]:
        # _loop1_208: (fstring | STRING)
        mark = self._mark()
        children = []
        while (
            (_tmp_368 := self._tmp_368())
        ):
            children.append(_tmp_368)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _tmp_209(self) -> Optional[Any]:
        # _tmp_209: star_named_expression ',' star_named_expressions?
        mark = self._mark()
        if (
    ...
    def _loop0_211(self) -> Optional[Any]:
        # _loop0_211: ',' double_starred_kvpair
        mark = self._mark()
        children = []
        while (
            (self.expect(','))
    ...
    def _gather_210(self) -> Optional[Any]:
        # _gather_210: double_starred_kvpair _loop0_211
        mark = self._mark()
        if (
            (elem := self.double_starred_kvpair())
            is not None
    ...
    @memoize
    def _loop1_212(self) -> Optional[Any]:
        # _loop1_212: for_if_clause
        mark = self._mark()
        children = []
        while (
    ...
    def _loop0_213(self) -> Optional[Any]:
        ...
        children = []
        while (
            (_tmp_369 := self._tmp_369())
        ):
            children.append(_tmp_369)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _loop0_214(self) -> Optional[Any]:
        ...
        children = []
        while (
            (_tmp_370 := self._tmp_370())
        ):
            children.append(_tmp_370)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    @memoize
    def _loop0_217(self) -> Optional[Any]:
        # _loop0_217: ',' (starred_expression | (assignment_expression | expression !':=') !'=')
        mark = self._mark()
        children = []
        while (
            (self.expect(','))
    ...
    @memoize
    def _gather_216(self) -> Optional[Any]:
        # _gather_216: (starred_expression | (assignment_expression | expression !':=') !'=') _loop0_217
        mark = self._mark()
        if (
            (elem := self._tmp_371())
            is not None
    ...
    def _gather_219(self) -> Optional[Any]:
        # _gather_219: kwarg_or_starred _loop0_220
        mark = self._mark()
        if (
            (elem := self.kwarg_or_starred())
            is not None
    ...
    def _loop0_222(self) -> Optional[Any]:
        # _loop0_222: ',' kwarg_or_double_starred
        mark = self._mark()
        children = []
        while (
            (self.expect(','))
    ...
    def _gather_221(self) -> Optional[Any]:
        # _gather_221: kwarg_or_double_starred _loop0_222
        mark = self._mark()
        if (
            (elem := self.kwarg_or_double_starred())
            is not None
    ...
    @memoize
    def _gather_223(self) -> Optional[Any]:
        # _gather_223: kwarg_or_starred _loop0_224
        mark = self._mark()
        if (
            (elem := self.kwarg_or_starred())
            is not None
    ...
    def _loop0_226(self) -> Optional[Any]:
        # _loop0_226: ',' kwarg_or_double_starred
        mark = self._mark()
        children = []
        while (
            (self.expect(','))
    ...
    @memoize
    def _gather_225(self) -> Optional[Any]:
        # _gather_225: kwarg_or_double_starred _loop0_226
        mark = self._mark()
        if (
            (elem := self.kwarg_or_double_starred())
            is not None
    ...
    def _loop0_227(self) -> Optional[Any]:
        # _loop0_227: (',' star_target)
        mark = self._mark()
        children = []
        while (
            (_tmp_372 := self._tmp_372())
        ):
            children.append(_tmp_372)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    @memoize
    def _gather_228(self) -> Optional[Any]:
        # _gather_228: star_target _loop0_229
        mark = self._mark()
        if (
            (elem := self.star_target())
            is not None
    ...
    @memoize
    def _loop1_230(self) -> Optional[Any]:
        # _loop1_230: (',' star_target)
        mark = self._mark()
        children = []
        while (
            (_tmp_373 := self._tmp_373())
        ):
            children.append(_tmp_373)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _gather_232(self) -> Optional[Any]:
        # _gather_232: del_target _loop0_233
        mark = self._mark()
        if (
            (elem := self.del_target())
            is not None
    ...
    @memoize
    def _gather_234(self) -> Optional[Any]:
        # _gather_234: expression _loop0_235
        mark = self._mark()
        if (
            (elem := self.expression())
            is not None
    ...
    def _gather_236(self) -> Optional[Any]:
        # _gather_236: expression _loop0_237
        mark = self._mark()
        if (
            (elem := self.expression())
            is not None
    ...
    def _tmp_243(self) -> Optional[Any]:
        # _tmp_243: args | expression for_if_clauses
        mark = self._mark()
        if (
            (args := self.args())
        ):
    ...
    def _tmp_248(self) -> Optional[Any]:
        # _tmp_248: NAME STRING | SOFT_KEYWORD
        mark = self._mark()
        if (
            (name := self.name())
            and
    ...
    def _tmp_250(self) -> Optional[Any]:
        # _tmp_250: FSTRING_MIDDLE | fstring_replacement_field
        mark = self._mark()
        if (
    ...
    def _tmp_252(self) -> Optional[Any]:
        # _tmp_252: list | tuple | genexp | 'True' | 'None' | 'False'
        mark = self._mark()
        if (
            (list := self.list())
        ):
    ...
    def _tmp_255(self) -> Optional[Any]:
        # _tmp_255: 'as' scenic_require_stmt_name
        mark = self._mark()
        if (
            (literal := self.expect('as'))
            and
    ...
    def _loop0_256(self) -> Optional[Any]:
        # _loop0_256: star_named_expressions
        mark = self._mark()
        children = []
        while (
    ...
    def _loop0_257(self) -> Optional[Any]:
        # _loop0_257: (star_targets '=')
        mark = self._mark()
        children = []
        while (
            (_tmp_374 := self._tmp_374())
        ):
            children.append(_tmp_374)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _loop0_258(self) -> Optional[Any]:
        # _loop0_258: (star_targets '=')
        mark = self._mark()
        children = []
        while (
            (_tmp_375 := self._tmp_375())
        ):
            children.append(_tmp_375)
            mark = self._mark()
        self._reset(mark)
        return children;
    ...
    def _tmp_263(self) -> Optional[Any]:
        # _tmp_263: slash_no_default | slash_with_default
        mark = self._mark()
        if (
    ...
    def _loop0_264(self) -> Optional[Any]:
        # _loop0_264: param_maybe_default
        mark = self._mark()
        children = []
        while (
            (param_maybe_default := self.param_maybe_default())
    ...
    @memoize
    def _loop0_265(self) -> Optional[Any]:
        # _loop0_265: param_no_default
        mark = self._mark()
        children = []
        while (
            (param_no_default := self.param_no_default())
    ...
    @memoize
    def _loop1_267(self) -> Optional[Any]:
        # _loop1_267: param_no_default
        mark = self._mark()
        children = []
        while (
            (param_no_default := self.param_no_default())
    ...
    def _tmp_268(self) -> Optional[Any]:
        # _tmp_268: slash_no_default | slash_with_default
        mark = self._mark()
        if (
            (slash_no_default := self.slash_no_default())
        ):
    ...
    @memoize
    def _loop0_269(self) -> Optional[Any]:
        # _loop0_269: param_maybe_default
        mark = self._mark()
        children = []
        while (
            (param_maybe_default := self.param_maybe_default())
    ...
    @memoize
    def _loop1_272(self) -> Optional[Any]:
        # _loop1_272: param_maybe_default
        mark = self._mark()
        children = []
        while (
            (param_maybe_default := self.param_maybe_default())
    ...
    def _tmp_274(self) -> Optional[Any]:
        ...
        if (
            (literal := self.expect(','))
            and
            (_tmp_376 := self._tmp_376())
        ):
            return [literal, _tmp_376];
        self._reset(mark)
        return None;

    @memoize
    def _tmp_275(self) -> Optional[Any]:
        # _tmp_275: param_no_default | ','
        mark = self._mark()
        if (
            (param_no_default := self.param_no_default())
        ):
    ...
    def _loop0_276(self) -> Optional[Any]:
        # _loop0_276: param_maybe_default
        mark = self._mark()
        children = []
        while (
            (param_maybe_default := self.param_maybe_default())
    ...
    def _tmp_280(self) -> Optional[Any]:
        # _tmp_280: lambda_slash_no_default | lambda_slash_with_default
        mark = self._mark()
        if (
    ...
    def _loop0_281(self) -> Optional[Any]:
        # _loop0_281: lambda_param_maybe_default
        mark = self._mark()
        children = []
        while (
            (lambda_param_maybe_default := self.lambda_param_maybe_default())
    ...
    def _loop0_282(self) -> Optional[Any]:
        # _loop0_282: lambda_param_no_default
        mark = self._mark()
        children = []
        while (
            (lambda_param_no_default := self.lambda_param_no_default())
    ...
    @memoize
    def _loop0_283(self) -> Optional[Any]:
        # _loop0_283: lambda_param_no_default
        mark = self._mark()
        children = []
        while (
            (lambda_param_no_default := self.lambda_param_no_default())
    ...
    @memoize
    def _loop0_285(self) -> Optional[Any]:
        # _loop0_285: ',' lambda_param
        mark = self._mark()
        children = []
        while (
            (self.expect(','))
    ...
    @memoize
    def _gather_284(self) -> Optional[Any]:
        # _gather_284: lambda_param _loop0_285
        mark = self._mark()
        if (
            (elem := self.lambda_param())
            is not None
    ...
    def _tmp_286(self) -> Optional[Any]:
        # _tmp_286: lambda_slash_no_default | lambda_slash_with_default
        mark = self._mark()
        if (
            (lambda_slash_no_default := self.lambda_slash_no_default())
        ):
    ...
    @memoize
    def _loop0_287(self) -> Optional[Any]:
        # _loop0_287: lambda_param_maybe_default
        mark = self._mark()
        children = []
        while (
            (lambda_param_maybe_default := self.lambda_param_maybe_default())
    ...
    def _tmp_288(self) -> Optional[Any]:
        # _tmp_288: ',' | lambda_param_no_default
        mark = self._mark()
        if (
            (literal := self.expect(','))
        ):
    ...
    @memoize
    def _loop0_289(self) -> Optional[Any]:
        # _loop0_289: lambda_param_maybe_default
        mark = self._mark()
        children = []
        while (
            (lambda_param_maybe_default := self.lambda_param_maybe_default())
    ...
    @memoize
    def _loop1_290(self) -> Optional[Any]:
        # _loop1_290: lambda_param_maybe_default
        mark = self._mark()
        children = []
        while (
            (lambda_param_maybe_default := self.lambda_param_maybe_default())
    ...
    @memoize
    def _loop1_291(self) -> Optional[Any]:
        # _loop1_291: lambda_param_with_default
        mark = self._mark()
        children = []
        while (
            (lambda_param_with_default := self.lambda_param_with_default())
    ...
    def _tmp_292(self) -> Optional[Any]:
        ...
        if (
            (literal := self.expect(','))
            and
            (_tmp_377 := self._tmp_377())
        ):
            return [literal, _tmp_377];
        self._reset(mark)
        return None;

    @memoize
    def _tmp_293(self) -> Optional[Any]:
        # _tmp_293: lambda_param_no_default | ','
        mark = self._mark()
        if (
            (lambda_param_no_default := self.lambda_param_no_default())
        ):
    ...
    def _loop0_294(self) -> Optional[Any]:
        # _loop0_294: lambda_param_maybe_default
        mark = self._mark()
        children = []
        while (
            (lambda_param_maybe_default := self.lambda_param_maybe_default())
    ...
    def _tmp_295(self) -> Optional[Any]:
        # _tmp_295: lambda_param_no_default | ','
        mark = self._mark()
        if (
            (lambda_param_no_default := self.lambda_param_no_default())
        ):
    ...
    @memoize
    def _loop0_299(self) -> Optional[Any]:
        # _loop0_299: ',' dotted_name
        mark = self._mark()
        children = []
        while (
            (self.expect(','))
    ...
    @memoize
    def _gather_298(self) -> Optional[Any]:
        # _gather_298: dotted_name _loop0_299
        mark = self._mark()
        if (
            (elem := self.dotted_name())
            is not None
    ...
    @memoize
    def _loop0_301(self) -> Optional[Any]:
        # _loop0_301: ',' (expression ['as' star_target])
        mark = self._mark()
        children = []
        while (
            (self.expect(','))
    ...
    @memoize
    def _gather_300(self) -> Optional[Any]:
        # _gather_300: (expression ['as' star_target]) _loop0_301
        mark = self._mark()
        if (
            (elem := self._tmp_378())
            is not None
    ...
    @memoize
    def _loop0_303(self) -> Optional[Any]:
        # _loop0_303: ',' (expressions ['as' star_target])
        mark = self._mark()
        children = []
        while (
            (self.expect(','))
    ...
    @memoize
    def _gather_302(self) -> Optional[Any]:
        # _gather_302: (expressions ['as' star_target]) _loop0_303
        mark = self._mark()
        if (
            (elem := self._tmp_379())
            is not None
    ...
    @memoize
    def _loop0_305(self) -> Optional[Any]:
        # _loop0_305: ',' (expression ['as' star_target])
        mark = self._mark()
        children = []
        while (
            (self.expect(','))
    ...
    @memoize
    def _gather_304(self) -> Optional[Any]:
        # _gather_304: (expression ['as' star_target]) _loop0_305
        mark = self._mark()
        if (
            (elem := self._tmp_380())
            is not None
    ...
    @memoize
    def _loop0_307(self) -> Optional[Any]:
        # _loop0_307: ',' (expressions ['as' star_target])
        mark = self._mark()
        children = []
        while (
            (self.expect(','))
    ...
    @memoize
    def _gather_306(self) -> Optional[Any]:
        # _gather_306: (expressions ['as' star_target]) _loop0_307
        mark = self._mark()
        if (
            (elem := self._tmp_381())
            is not None
    ...
    @memoize
    def _loop0_309(self) -> Optional[Any]:
        # _loop0_309: block
        mark = self._mark()
        children = []
        while (
    ...
    @memoize
    def _loop1_310(self) -> Optional[Any]:
        # _loop1_310: except_block
        mark = self._mark()
        children = []
        while (
            (except_block := self.except_block())
    ...
    @memoize
    def _loop0_312(self) -> Optional[Any]:
        # _loop0_312: block
        mark = self._mark()
        children = []
        while (
    ...
    @memoize
    def _loop1_313(self) -> Optional[Any]:
        # _loop1_313: except_star_block
        mark = self._mark()
        children = []
        while (
            (except_star_block := self.except_star_block())
    ...
    @memoize
    def _loop0_325(self) -> Optional[Any]:
        # _loop0_325: ',' double_starred_kvpair
        mark = self._mark()
        children = []
        while (
            (self.expect(','))
    ...
    @memoize
    def _gather_324(self) -> Optional[Any]:
        # _gather_324: double_starred_kvpair _loop0_325
        mark = self._mark()
        if (
            (elem := self.double_starred_kvpair())
            is not None
    ...
    def _tmp_328(self) -> Optional[Any]:
        # _tmp_328: yield_expr | star_expressions
        mark = self._mark()
        if (
            (yield_expr := self.yield_expr())
        ):
    ...
    def _tmp_329(self) -> Optional[Any]:
        # _tmp_329: yield_expr | star_expressions
        mark = self._mark()
        if (
            (yield_expr := self.yield_expr())
        ):
    ...
    def _tmp_331(self) -> Optional[Any]:
        # _tmp_331: yield_expr | star_expressions
        mark = self._mark()
        if (
            (yield_expr := self.yield_expr())
        ):
    ...
    def _tmp_333(self) -> Optional[Any]:
        # _tmp_333: yield_expr | star_expressions
        mark = self._mark()
        if (
            (yield_expr := self.yield_expr())
        ):
    ...
    def _tmp_334(self) -> Optional[Any]:
        # _tmp_334: yield_expr | star_expressions
        mark = self._mark()
        if (
            (yield_expr := self.yield_expr())
        ):
    ...
    def _tmp_337(self) -> Optional[Any]:
        # _tmp_337: yield_expr | star_expressions
        mark = self._mark()
        if (
            (yield_expr := self.yield_expr())
        ):
    ...
    @memoize
    def _loop0_339(self) -> Optional[Any]:
        # _loop0_339: fstring_format_spec
        mark = self._mark()
        children = []
        while (
            (fstring_format_spec := self.fstring_format_spec())
    ...
    def _tmp_340(self) -> Optional[Any]:
        # _tmp_340: yield_expr | star_expressions
        mark = self._mark()
        if (
            (yield_expr := self.yield_expr())
        ):
    ...
    @memoize
    def _loop0_347(self) -> Optional[Any]:
        # _loop0_347: ',' scenic_class_property_attribute
        mark = self._mark()
        children = []
        while (
            (self.expect(','))
    ...
    @memoize
    def _gather_346(self) -> Optional[Any]:
        # _gather_346: scenic_class_property_attribute _loop0_347
        mark = self._mark()
        if (
            (elem := self.scenic_class_property_attribute())
            is not None
    ...
    def _tmp_349(self) -> Optional[Any]:
        # _tmp_349: scenic_specifiers ',' NEWLINE
        mark = self._mark()
        if (
    ...
    def _tmp_350(self) -> Optional[Any]:
        # _tmp_350: scenic_specifiers ',' NEWLINE
        mark = self._mark()
        if (
            (x := self.scenic_specifiers())
            and
    ...
    def _tmp_351(self) -> Optional[Any]:
        # _tmp_351: (scenic_precondition_stmt | scenic_invariant_stmt) NEWLINE
        mark = self._mark()
        if (
            (x := self._tmp_383())
            and
    ...
    def _tmp_353(self) -> Optional[Any]:
        # _tmp_353: scenic_contract_test_specs ',' NEWLINE
        mark = self._mark()
        if (
            (x := self.scenic_contract_test_specs())
            and
    ...
    @memoize
    def _tmp_354(self) -> Optional[Any]:
        # _tmp_354: scenic_contract_test_specs ',' NEWLINE
        mark = self._mark()
        if (
            (x := self.scenic_contract_test_specs())
            and
    ...
    @memoize
    def _tmp_355(self) -> Optional[Any]:
    ...
    @memoize
    def _tmp_356(self) -> Optional[Any]:
        # _tmp_356: star_targets '='
        mark = self._mark()
        if (
            (z := self.star_targets())
            and
    ...
    @memoize
    def _tmp_357(self) -> Optional[Any]:
        # _tmp_357: scenic_contract_assume_specs ',' NEWLINE
        mark = self._mark()
        if (
            (x := self.scenic_contract_assume_specs())
            and
    ...
    @memoize
    def _tmp_358(self) -> Optional[Any]:
        # _tmp_358: scenic_contract_assume_specs ',' NEWLINE
        mark = self._mark()
        if (
            (x := self.scenic_contract_assume_specs())
            and
    ...
    @memoize
    def _tmp_359(self) -> Optional[Any]:
        # _tmp_359: star_targets '='
        mark = self._mark()
        if (
            (z := self.star_targets())
            and
    ...
    @memoize
    def _tmp_360(self) -> Optional[Any]:
        # _tmp_360: star_targets '='
        mark = self._mark()
        if (
            (z := self.star_targets())
            and
    ...
    @memoize
    def _tmp_361(self) -> Optional[Any]:
        # _tmp_361: ',' expression
        mark = self._mark()
        if (
            (self.expect(','))
    ...
    @memoize
    def _tmp_362(self) -> Optional[Any]:
        # _tmp_362: ',' star_expression
        mark = self._mark()
        if (
            (self.expect(','))
            and
    ...
    @memoize
    def _tmp_363(self) -> Optional[Any]:
        # _tmp_363: 'or' conjunction
        mark = self._mark()
        if (
            (self.expect('or'))
            and
    ...
    @memoize
    def _tmp_364(self) -> Optional[Any]:
        # _tmp_364: 'or' (scenic_temporal_prefix | scenic_temporal_conjunction)
        mark = self._mark()
        if (
            (self.expect('or'))
            and
    ...
    @memoize
    def _tmp_365(self) -> Optional[Any]:
        # _tmp_365: 'and' inversion
        mark = self._mark()
        if (
            (self.expect('and'))
            and
    ...
    @memoize
    def _tmp_366(self) -> Optional[Any]:
        # _tmp_366: 'and' (scenic_temporal_prefix | scenic_temporal_inversion)
        mark = self._mark()
        if (
            (self.expect('and'))
            and
    ...
    @memoize
    def _tmp_367(self) -> Optional[Any]:
        # _tmp_367: slice | starred_expression
        mark = self._mark()
        if (
            (slice := self.slice())
        ):
    ...
    @memoize
    def _tmp_368(self) -> Optional[Any]:
        # _tmp_368: fstring | STRING
        mark = self._mark()
        if (
    ...
    @memoize
    def _tmp_369(self) -> Optional[Any]:
        # _tmp_369: 'if' disjunction
        mark = self._mark()
        if (
            (self.expect('if'))
            and
    ...
    @memoize
    def _tmp_370(self) -> Optional[Any]:
        # _tmp_370: 'if' disjunction
        mark = self._mark()
        if (
            (self.expect('if'))
            and
    ...
    @memoize
    def _tmp_371(self) -> Optional[Any]:
        # _tmp_371: starred_expression | (assignment_expression | expression !':=') !'='
        mark = self._mark()
        if (
            (starred_expression := self.starred_expression())
        ):
    ...
    @memoize
    def _tmp_372(self) -> Optional[Any]:
        # _tmp_372: ',' star_target
        mark = self._mark()
        if (
            (self.expect(','))
            and
    ...
    @memoize
    def _tmp_373(self) -> Optional[Any]:
        # _tmp_373: ',' star_target
        mark = self._mark()
        if (
            (self.expect(','))
            and
    ...
    @memoize
    def _tmp_374(self) -> Optional[Any]:
    ...
    @memoize
    def _tmp_375(self) -> Optional[Any]:
        # _tmp_375: star_targets '='
        mark = self._mark()
        if (
            (star_targets := self.star_targets())
            and
    ...
    @memoize
    def _tmp_376(self) -> Optional[Any]:
        # _tmp_376: ')' | '**'
        mark = self._mark()
        if (
            (literal := self.expect(')'))
    ...
    @memoize
    def _tmp_377(self) -> Optional[Any]:
        # _tmp_377: ':' | '**'
        mark = self._mark()
        if (
            (literal := self.expect(':'))
    ...
    @memoize
    def _tmp_378(self) -> Optional[Any]:
        # _tmp_378: expression ['as' star_target]
        mark = self._mark()
        if (
            (expression := self.expression())
            and
    ...
    @memoize
    def _tmp_379(self) -> Optional[Any]:
        # _tmp_379: expressions ['as' star_target]
        mark = self._mark()
        if (
    ...
    @memoize
    def _tmp_380(self) -> Optional[Any]:
        # _tmp_380: expression ['as' star_target]
        mark = self._mark()
        if (
            (expression := self.expression())
            and
    ...
    @memoize
    def _tmp_381(self) -> Optional[Any]:
        # _tmp_381: expressions ['as' star_target]
        mark = self._mark()
        if (
            (expressions := self.expressions())
            and
    ...
    @memoize
    def _tmp_382(self) -> Optional[Any]:
        # _tmp_382: 'as' NAME
        mark = self._mark()
        if (
            (literal := self.expect('as'))
    ...
    @memoize
    def _tmp_383(self) -> Optional[Any]:
        # _tmp_383: scenic_precondition_stmt | scenic_invariant_stmt
        mark = self._mark()
        if (
    ...
    @memoize
    def _tmp_384(self) -> Optional[Any]:
        # _tmp_384: scenic_temporal_prefix | scenic_temporal_conjunction
        mark = self._mark()
        if (
            (scenic_temporal_prefix := self.scenic_temporal_prefix())
        ):
    ...
    @memoize
    def _tmp_385(self) -> Optional[Any]:
        # _tmp_385: scenic_temporal_prefix | scenic_temporal_inversion
        mark = self._mark()
        if (
            (scenic_temporal_prefix := self.scenic_temporal_prefix())
        ):
    ...
    @memoize
    def _tmp_386(self) -> Optional[Any]:
        # _tmp_386: assignment_expression | expression !':='
        mark = self._mark()
        if (
            (assignment_expression := self.assignment_expression())
        ):
    ...
    @memoize
    def _tmp_387(self) -> Optional[Any]:
        # _tmp_387: 'as' star_target
        mark = self._mark()
        if (
            (literal := self.expect('as'))
            and
    ...
    @memoize
    def _tmp_388(self) -> Optional[Any]:
        # _tmp_388: 'as' star_target
        mark = self._mark()
        if (
            (literal := self.expect('as'))
            and
    ...
    @memoize
    def _tmp_389(self) -> Optional[Any]:
        # _tmp_389: 'as' star_target
        mark = self._mark()
        if (
            (literal := self.expect('as'))
            and
    ...
    @memoize
    def _tmp_390(self) -> Optional[Any]:
        # _tmp_390: 'as' star_target
        mark = self._mark()
        if (
            (literal := self.expect('as'))
            and
    ...
    SOFT_KEYWORDS = ('_', 'abort', 'above', 'actions', 'additive', 'after', 'ahead', 'along', 'altitude', 'always', 'angle', 'apparent', 'apparently', 'as', 'assume', 'assumptions', 'away', 'back', 'behavior', 'behind', 'below', 'beyond', 'body', 'bottom', 'can', 'case', 'choose', 'component', 'compose', 'confidence', 'connect', 'contained', 'contract', 'correctness', 'definitions', 'deg', 'directly', 'distance', 'dynamic', 'ego', 'eventually', 'facing', 'final', 'follow', 'following', 'from', 'front', 'gap', 'globals', 'guarantees', 'heading', 'hours', 'implement', 'implies', 'initial', 'inputs', 'interrupt', 'intersects', 'invariant', 'left', 'match', 'merge', 'minutes', 'model', 'monitor', 'mutate', 'next', 'not', 'objects', 'of', 'offset', 'outputs', 'over', 'override', 'param', 'past', 'position', 'precondition', 'prove', 'record', 'refine', 'relative', 'requiring', 'right', 'samples', 'satisfies', 'scenario', 'seconds', 'see', 'sensors', 'setup', 'shuffle', 'simulation', 'simulator', 'state', 'steps', 'take', 'terminate', 'terminating', 'test', 'top', 'toward', 'type', 'use', 'using', 'verify', 'visible', 'wait', 'weak', 'when', 'with', 'workspace')
